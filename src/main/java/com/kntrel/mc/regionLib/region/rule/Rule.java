...
/**
 * A {@link RegionListener} that reacts to a rule value stored in a region.
 * <p>
 * Rules model: "If something happens inside a region, use the rule value under
 * this rule name to determine what to do." When a trigger fires, the rule is
 * invoked with the region's current value (or via {@link #fireOnAbsent(Event, Region)}
 * if no value exists).
 *
